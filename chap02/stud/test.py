...
def compile(fname, text):
    with open(fname + '.asm', 'w') as f:
        f.write(text)

    ok1 = subprocess.call(['nasm', '-f', 'elf64', fname + '.asm', '-o', fname+'.o']) == 0
    ok2 = subprocess.call(['ld', '-o', fname, fname+'.o']) == 0
...
def launch(fname, seed=''):
    try:
        p = Popen(['./'+fname], shell=False, stdin=PIPE, stdout=PIPE)
        # Python3：入力は bytes にする
